...
* Ветка dev-1.4 - основная.
* Ветка t-astra - для ОС Astra Linux, она наследует ветку dev-1.4. В ветке t-astra не поддерживается измеритель ASA.
* Ветка dev-1.5 наследует ветку dev-1.4. В ветке dev-1.5 добавлена поддержка кнопок на измерителях IVMeasurerIVM.

Все общие изменения нужно вносить сначала в ветку dev-1.4, а потом сливать в t-astra и dev-1.5.
...
2. Если при подключении мультиплексор не обнаруживается, то стоит прописать в системе VID и PID устройства для драйвера виртуального COM-порта:

   ```bash
   sudo sh -c 'echo 1CBC 0008 > /sys/bus/usb/drivers/cdc_acm/new_id'
   ```

3. Для корректной работы ПО с COM-портами пользователь должен находиться в группе **dialout**. Чтобы добавить текущего пользователя в эту группу, выполните команду:
...
5. Если у вас возникла такая ошибка:
