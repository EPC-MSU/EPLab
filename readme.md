...
- Для корректной работы приложения на виртуальной машине нужно настроить сеть виртуальной машины. Для этого:

  ```bash
  Меню -> Устройства -> Сеть -> Настроить сеть -> Тип подключения -> Сетевой мост
  ```

  После настройки сети нужно перезагрузить виртуальную машину, чтобы настройки вступили в силу.
